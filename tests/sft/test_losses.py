...
def test_compute_sft_loss_components_adds_weighted_rationale_loss():
    batch_size = 2
    seq_len = 5
    vocab_size = 8
    input_ids = torch.tensor(
    ...
    weights = torch.tensor([0.75, 0.25], dtype=torch.float)

    components = compute_sft_loss_components(
        logits,
        input_ids,
        concept_mask,
        group_ids,
    ...
    expected_concept_loss = compute_sft_loss(
        logits,
        input_ids,
        concept_mask,
        group_ids,
    ...
    expected_total = expected_concept_loss + 0.2 * components.rationale_loss
    components.total_loss.backward()

    assert torch.allclose(components.concept_loss, expected_concept_loss, atol=1e-6)
    assert torch.allclose(components.total_loss, expected_total, atol=1e-6)
    assert components.rationale_loss.item() >= 0.0
    assert logits.grad is not None
